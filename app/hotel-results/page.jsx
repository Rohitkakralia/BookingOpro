  ...
  const performSearch = async () => {
    ...
    try {
      ...
      if (response.success) {
        ...
        const hotelsArray =
          response.data.data?.hotels || response.data.hotels || [];
  ...
  const handleViewHotelInfo = async (hotelId) => {
    ...
    try {
      ...
      if (response.success) {
        ...
        setHotelInfoError(
          response.message || "Failed to fetch hotel information"
        );
      }
    } catch (err) {
      ...
      <div className="min-h-screen bg-gray-50 flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-gray-600 mb-6">
            Please perform a search to view hotel results.
          </p>
          <a
            href="/Hotels"
      ...
      <div className="container mx-auto py-8 px-6">
        ...
        {hotels && hotels.length > 0 && (
          <div>
            <div className="text-gray-600 mb-6 text-lg">
              Found {hotels.length} hotel{hotels.length !== 1 ? "s" : ""} in{" "}
              {destination}
            </div>
        ...
        {searchResults && hotels.length === 0 && !loading && (
          <div className="bg-yellow-100 border border-yellow-400 text-yellow-700 px-4 py-3 rounded">
            ℹ️ No hotels found for your search criteria. Try different dates or
            destination.
          </div>
        )}
        ...
        {selectedHotelInfo && (
          <HotelInfoModal
            hotel={selectedHotelInfo}
            onClose={closeHotelInfoModal}
          />
        )}
      </div>
    </div>
